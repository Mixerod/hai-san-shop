# Module 2: Dashboard Screen & Responsive Grid

## 📌 Kế hoạch Kiểm thử & Sửa lỗi
- [ ] 1. Kiểm tra hiển thị của 3 ô chỉ số (Thống kê Doanh thu, Đơn hàng, Khách hàng).
- [ ] 2. Kiểm tra định dạng tiền tệ Việt Nam (vi-VN) cho ô Doanh thu.
- [ ] 3. Kiểm tra tính năng Responsive:
  - Trên màn hình điện thoại (width < 768): 3 thẻ chỉ số hiển thị dạng danh sách dọc (hoặc 1-2 cột tùy tỷ lệ).
  - Trên màn hình máy tính bảng (width >= 768): 3 thẻ chỉ số hiển thị thành hàng ngang (grid 3 cột).
- [ ] 4. Kiểm tra nút "Refresh" góc trên bên phải:
  - Khi click -> Trạng thái xoay tròn của Icon hoạt động.
  - Tải lại dữ liệu mới nhất từ Supabase.
- [ ] 5. Kiểm tra tính năng lọc biểu đồ hoặc thống kê theo khoảng thời gian (nếu có).
- [ ] 6. Kiểm tra xử lý lỗi khi không có kết nối mạng (Network Error) -> hiển thị thông báo thay vì sập màn hình.

---

## 📝 Nhật ký Kiểm thử & Nhật ký Sửa lỗi
*(Tiến độ kiểm thử sẽ được cập nhật tại đây khi tiến hành chạy test)*
