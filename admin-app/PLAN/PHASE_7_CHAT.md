# Phase 7 — Feedbacks & Live Chat

## Mục tiêu
Màn hình danh sách conversations (group theo identifier), màn hình chat với từng khách, gửi reply, mark đọc, badge số chưa đọc.
...
```
Table: feedbacks
  id, title (identifier của khách), content (nội dung tin), rating (null nếu là chat),
  is_read, created_at

Logic:
- Tin từ khách: is_read = false ban đầu
- Tin reply từ admin: title thường có prefix '[ADMIN]' hoặc identifier khác
- Để group conversations: nhóm theo title (identifier)
- Identifier của khách = số điện thoại hoặc "guest_name_phone" hoặc email

Query danh sách conversations:
  SELECT DISTINCT title, MAX(created_at), COUNT(*) FILTER (WHERE NOT is_read)
  FROM feedbacks GROUP BY title ORDER BY MAX(created_at) DESC

Query tin nhắn trong conversation:
  SELECT * FROM feedbacks WHERE title = :identifier ORDER BY created_at ASC
```

## Bước 1 — Tạo màn hình Chat list + Chat detail

Thay toàn bộ `/home/user/hai-san-shop/admin-app/app/(tabs)/chat.tsx`:
...
// Phát hiện admin message bằng prefix
...
export default function ChatScreen() {
  ...
  async function fetchConversations() {
    ...
    for (const row of data) {
      ...
      if (id.startsWith(ADMIN_PREFIX)) continue; // skip admin messages in list
      if (!map.has(id)) {
        map.set(id, {
          identifier: id,
            ...
            <View style={[s.bubble, item.isAdmin ? s.bubbleAdmin : s.bubbleCustomer]}>
              {item.rating != null && (
                <View style={s.ratingRow}>
                  {[1,2,3,4,5].map(n => (
                    <Star key={n} color={n <= item.rating! ? '#f59e0b' : '#374151'} size={14} fill={n <= item.rating! ? '#f59e0b' : 'transparent'} />
                  ))}
                </View>
              )}
          ...
          <TouchableOpacity style={s.convCard} onPress={() => openConversation(item.identifier)}>
            ...
            <View style={s.convContent}>
              ...
              {item.rating != null && (
                <View style={s.ratingRow}>
                  {[1,2,3,4,5].map(n => (
                    <Star key={n} color={n <= item.rating! ? '#f59e0b' : '#374151'} size={12} fill={n <= item.rating! ? '#f59e0b' : 'transparent'} />
                  ))}
                </View>
              )}
            </View>
...
## Bước 2 — Thêm badge vào Tab icon

Cập nhật `/home/user/hai-san-shop/admin-app/app/(tabs)/_layout.tsx` để hiện badge unread trên tab Chat.

Thêm state và logic đọc unread count từ Supabase vào tab layout, hoặc dùng cách đơn giản hơn: để màn hình chat tự hiển thị badge bên trong header.

(Giải pháp đơn giản: badge đã hiện trong header của ChatScreen rồi.)
...
git add admin-app/
git commit -m "feat: live chat with reply, mark read, polling"
git push -u origin claude/seafood-admin-app-Gd7JI
```

## Verification
- Danh sách conversations hiện tên khách và preview tin nhắn
- Badge số chưa đọc hiện trên mỗi conversation
- Mở conversation → xem toàn bộ lịch sử chat
- Nhập và gửi reply → tin hiện ngay
- Admin messages hiển thị bên phải, khách bên trái
- Mark all read hoạt động

## NEXT: Phase 8
Sau khi xong, đọc `PHASE_8_BATCH.md` và thực thi.
