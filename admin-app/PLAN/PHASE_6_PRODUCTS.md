# Phase 6 — Quản lý Sản phẩm

## Mục tiêu
Màn hình danh sách sản phẩm với tìm kiếm, toggle in_stock nhanh, thêm/sửa/xoá sản phẩm, và upload ảnh.
...
```
Table: products
  id, name, description, price, original_price, unit, image_url,
  in_stock, category, tag, note, total_sold, created_at

Storage bucket: haisanshop (public)
Upload path: products/{timestamp}_{filename}

Categories: haisan | cá tươi | cá khô | tôm | cua | mực | chả | nước mắm | khác
Tags: none | new | best_seller | rare | premium
Units: kg | con | hộp | gói | kg/con
```

## Bước 1 — Thay thế màn hình Products

Thay toàn bộ `/home/user/hai-san-shop/admin-app/app/(tabs)/products.tsx`:
...
git add admin-app/
git commit -m "feat: products management with image upload"
git push -u origin claude/seafood-admin-app-Gd7JI
```

## Verification
- Danh sách sản phẩm hiện ra với ảnh, giá, tag
- Toggle in_stock hoạt động
- Modal thêm/sửa sản phẩm hoạt động
- Upload ảnh từ gallery và lưu lên Supabase Storage
- Xoá sản phẩm có confirm dialog

## NEXT: Phase 7
Sau khi xong, đọc `PHASE_7_CHAT.md` và thực thi.
