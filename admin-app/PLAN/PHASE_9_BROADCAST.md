...
## Mục tiêu
Màn hình gửi thông báo hàng loạt cho tất cả khách hàng. Thông báo được lưu vào bảng `notifications` và hiện trong notification bell của web app.

## Prerequisites
- Phase 1 & 2 đã hoàn thành
- Phase 6 (Products) nên xong trước vì UI broadcast thường đi kèm sản phẩm
...
Types:
  ...
  price_change → thông báo thay đổi giá
```

## Bước 1 — Tạo màn hình Broadcast

Tạo file mới `/home/user/hai-san-shop/admin-app/app/broadcast.tsx`:
...
## Bước 2 — Thêm nút Broadcast vào Dashboard

Cập nhật `/home/user/hai-san-shop/admin-app/app/(tabs)/dashboard.tsx`:

Thêm nút này vào cuối ScrollView trong DashboardScreen:

```typescript
import { useRouter } from 'expo-router';
// ...
const router = useRouter();
// Trong JSX, thêm vào cuối content ScrollView:
<TouchableOpacity
...
git add admin-app/
git commit -m "feat: broadcast notifications screen with history"
git push -u origin claude/seafood-admin-app-Gd7JI
```

## Verification
- Màn hình Broadcast mở từ Dashboard
- Chọn loại thông báo (general/new_product/price_change)
- Nhập nội dung → Gửi → confirm dialog
- Thông báo lưu vào Supabase `notifications` table
- Lịch sử hiện 20 thông báo gần nhất
- Web app hiện notification bell khi có thông báo mới

## ✅ HOÀN THÀNH TẤT CẢ CÁC PHASE

Cập nhật MASTER_PLAN.md, đánh dấu tất cả checkbox và commit lần cuối:

```bash
cd /home/user/hai-san-shop
# Cập nhật MASTER_PLAN.md: đánh dấu tất cả [ ] thành [x]
git add admin-app/PLAN/MASTER_PLAN.md
git commit -m "docs: mark all phases complete in MASTER_PLAN"
git push -u origin claude/seafood-admin-app-Gd7JI
```
