...
import { Bell, Megaphone, Save, Send, ChevronDown, ChevronUp, Check, Loader2, ShoppingCart, Zap, Plus, Minus, X, Search } from 'lucide-react'
...
export default function ProductsPage() {
  ...
  const handleInputBlur = (productId: string, unit: string) => {
    const valStr = quantities[productId]
    if (valStr !== undefined) {
      ...
      setQuantities(prev => ({ ...prev, [productId]: String(val) }))
    }
  }
  ...
  return (
    ...
    <main className="w-full max-w-6xl font-sans">
      ...
      {/* Bộ lọc danh mục & Tìm kiếm bám dính (Sticky Filter & Search Bar) */}
      <div className="sticky top-[68px] sm:top-[80px] z-30 bg-slate-50/90 backdrop-blur-md py-4 -mx-4 px-4 sm:-mx-6 sm:px-6 border-b border-slate-100 flex flex-row gap-3 items-center justify-between mb-8 shadow-sm transition-all duration-300">
        
        {/* Lọc danh mục dạng Dropdown nút viên thuốc cực gọn */}
        <div className="relative">
          <button
            type="button"
            onClick={() => setShowCategoryDropdown(!showCategoryDropdown)}
            className="bg-white hover:bg-slate-50 border border-slate-200/80 hover:border-slate-350 text-slate-700 font-extrabold text-xs sm:text-sm px-4 sm:px-5 py-3 rounded-2xl transition-all shadow-sm hover:shadow active:scale-95 flex items-center gap-2 cursor-pointer select-none"
          >
            <span className="text-blue-500 text-sm">🐟</span>
            <span>Danh mục: <strong className="text-blue-600 font-black">{selectedCategory}</strong></span>
            <ChevronDown className={`w-3.5 h-3.5 text-slate-400 transition-transform duration-350 ${showCategoryDropdown ? 'rotate-180' : ''}`} />
          </button>
          ...
          {showCategoryDropdown && (
            <div className="absolute top-full left-0 mt-2 bg-white/95 backdrop-blur-md border border-slate-200/80 rounded-2xl shadow-xl shadow-slate-900/10 p-3 sm:p-4 w-[280px] sm:w-[380px] max-w-[92vw] z-50 animate-in fade-in slide-in-from-top-3 duration-250">
              <p className="text-[9px] text-slate-400 font-black uppercase tracking-wider mb-2.5 px-1">Chọn loại hải sản cần lọc</p>
                ...
                {categories.map((cat) => {
                  ...
                  return (
                    <button
                      ...
                      className={`px-3 py-2.5 rounded-xl text-[11px] font-black transition-all duration-200 cursor-pointer select-none flex items-center justify-between border ${
                        isSelected
        ...
        {/* Thanh tìm kiếm nhanh có Auto-suggest */}
        <div className="relative flex-1 md:flex-none md:w-[320px] shrink-0 z-40">
          <div className="relative flex items-center">
            <input
              ...
              className="w-full bg-white border border-slate-200 hover:border-slate-350 focus:border-blue-500 focus:bg-white rounded-2xl py-3 text-xs font-semibold text-slate-800 placeholder-slate-400 outline-none focus:ring-4 focus:ring-blue-500/10 transition-all shadow-sm"
            />
            ...
            {searchQuery && (
              <button
                ...
                className="absolute right-3 p-1 rounded-full text-slate-450 hover:text-slate-600 hover:bg-slate-100 transition-all cursor-pointer"
              >
          ...
          {/* Underlay to close auto-suggest dropdown when clicked outside */}
          ...
          {/* Auto-suggest dropdown floating overlay panel */}
          {showSuggest && searchQuery && (
            <div className="absolute top-full left-0 right-0 mt-2 bg-white/95 backdrop-blur-md border border-slate-200/80 rounded-2xl shadow-xl shadow-slate-900/10 p-3 z-40 max-h-[300px] overflow-y-auto animate-in fade-in slide-in-from-top-2 duration-200">
              <div className="flex justify-between items-center mb-2 px-1">
                <span className="text-[9px] text-slate-400 font-black uppercase tracking-wider">Gợi ý hải sản</span>
                <span className="text-[9px] text-blue-500 font-bold">Khớp với từ khóa</span>
              </div>
              <div className="space-y-1">
                {products
                  ...
                  .map(p => (
                    <button
                      ...
                      className="w-full text-left px-2.5 py-2 hover:bg-blue-50/40 rounded-xl flex items-center gap-3 transition-colors cursor-pointer group"
                    >
                      <div className="w-9 h-9 rounded-xl bg-slate-50 overflow-hidden shrink-0 border border-slate-100 flex items-center justify-center text-lg font-bold">
                        {p.image_url ? (
                          <img src={p.image_url} alt={p.name} className="w-full h-full object-cover" />
                        ) : '🐟'}
                      </div>
                      <div className="flex-1 min-w-0">
                        <p className="text-xs font-extrabold text-slate-800 truncate group-hover:text-blue-600 transition-colors">{p.name}</p>
                        <p className="text-[10px] text-slate-500 font-bold uppercase tracking-wide">
                          {p.price.toLocaleString('vi-VN')}đ<span className="text-slate-400 font-medium lowercase">/{p.unit}</span>
                        </p>
                      </div>
                    </button>
                  ))
                ...
                {products.filter(p => p.name.toLowerCase().includes(searchQuery.toLowerCase())).length === 0 && (
                  <div className="text-center py-6 text-slate-400">
                    ...
                    <p className="text-xs font-semibold">Không tìm thấy gợi ý nào</p>
                  </div>
                )}
              </div>
            </div>
        ...
        <div className="bg-white border border-slate-100 rounded-3xl p-16 text-center shadow-sm w-full">
          ...
          <p className="text-slate-500 font-bold text-lg">
            {searchQuery 
              ...
              : `Không tìm thấy sản phẩm nào trong danh mục "${selectedCategory}"`}
          </p>
        </div>
      ) : (
