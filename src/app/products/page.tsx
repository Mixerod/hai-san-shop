      ...
      {products.length === 0 ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
          {products.map((p) => (
            <div 
              key={p.id} 
              className="bg-white border border-slate-100 rounded-3xl overflow-hidden shadow-sm hover:shadow-2xl hover:shadow-blue-900/5 hover:-translate-y-2 hover:border-blue-500/10 transition-all duration-500 flex flex-col group relative"
            >
              {/* Image Container with Zoom effect */}
              <div className="overflow-hidden w-full h-56 bg-slate-50 relative border-b border-slate-100/50">
                {p.image_url ? (
                  <img 
                    src={p.image_url} 
                    alt={p.name} 
                  ...
                  <div className="w-full h-full flex items-center justify-center text-6xl group-hover:scale-112 group-hover:rotate-3 transition-all duration-500">
                    🐟
                  </div>
                )}
                {!p.in_stock && (
                  <div className="absolute inset-0 bg-slate-900/60 backdrop-blur-[3px] flex items-center justify-center z-10">
                    <span className="bg-red-500 text-white px-5 py-2 rounded-full font-black text-xs uppercase tracking-widest shadow-lg shadow-red-500/30">
                      Tạm hết hàng
                    </span>
                  </div>
                )}
              ...
              {/* Product Info Block */}
              <div className="p-6 flex flex-col flex-1 relative z-20 bg-white transition-all duration-300">
                <div className="flex justify-between items-start mb-3">
                  <h2 className="text-xl font-bold text-slate-800 line-clamp-2 leading-snug group-hover:text-blue-600 transition-colors">
                    {p.name}
                  </h2>
                </div>
                ...
                {p.note && (
                  <div className="bg-amber-50 border border-amber-100/70 rounded-xl p-3 mb-4 inline-flex items-start gap-2">
                    <span className="text-xs mt-0.5">💡</span>
                    <p className="text-amber-700 text-xs font-semibold leading-relaxed">{p.note}</p>
                  </div>
                )}
                ...
                <div className="mt-auto pt-4 border-t border-slate-50">
                  <div className="flex justify-between items-end mb-5">
                    <div className="flex-1">
                      <p className="text-[10px] text-slate-400 font-bold mb-1.5 uppercase tracking-wider">Đơn giá</p>
                      {isAdmin ? (
                        <div className="flex items-center gap-2">
                          <div className="relative flex items-center">
                            <input
                              ...
                              className="w-24 bg-blue-50/60 border border-blue-200/80 rounded-xl px-2.5 py-1.5 text-blue-700 font-extrabold text-sm focus:outline-none focus:border-blue-500 focus:ring-4 focus:ring-blue-500/10 transition-all text-center shadow-sm"
                            />
                            <span className="absolute right-2 text-slate-450 text-xs font-bold pointer-events-none">đ</span>
                          </div>
                          
                          <button
                            onClick={() => handleSavePrice(p.id, p)}
                            disabled={savingId === p.id}
                            className="p-2 bg-emerald-500 hover:bg-emerald-600 active:scale-90 text-white rounded-xl transition-all shadow-md shadow-emerald-500/25 border border-emerald-600/10 cursor-pointer animate-in fade-in"
                            title="Lưu giá mới"
                          >
                            {savingId === p.id ? (
                    ...
                    <p className="text-slate-500 text-xs font-bold bg-slate-50 px-3 py-1.5 rounded-lg border border-slate-100 group-hover:bg-blue-50 group-hover:text-blue-600 transition-colors self-end shrink-0">
                      Đã bán: {p.total_sold}
                    </p>
                  </div>
                  ...
                  <button
                    disabled={!p.in_stock}
                    onClick={() => openModal(p)}
                    className="w-full bg-orange-500 hover:bg-orange-600 active:scale-95 disabled:bg-slate-100 disabled:text-slate-400 disabled:active:scale-100 disabled:cursor-not-allowed text-white font-extrabold py-3.5 rounded-xl transition-all shadow-md shadow-orange-500/10 hover:shadow-lg hover:shadow-orange-500/25 flex items-center justify-center gap-2 border border-orange-600/10"
                  >
                    {p.in_stock ? (
                      <>
